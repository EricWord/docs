...
1. 堆溢出
   ...
   java堆用于存储对象实例，只要不断地创建对象，并且对象不会被回收，当对象的数量达到一定的数量进而达到了最大堆容量限制就会产生内存溢出
...
1. 对于RDD来说，每个分片都会被一个计算任务处理，分片数决定并行度

2. 一个函数会被作用在每一个分区

3. 一个RDD会依赖于其他多个RDD

4. 对于KV类型的RDD会有一个Partitioner,即RDD的分区函数

5. 存储了每个Partition优先位置的列表
...
- Taildir Source
  ...
  监控指定的多个文件，一旦文件内有新写入的数据，就会将其写入到指定的sink内，本来源可靠性高，不会丢失数据
...
小文件产生的原因是， 写入HDFS文件中断等因素， 小文件一直处于open write的状态，没有正确关闭，导致hive无法读取。
...
相同数据量的情况下，二叉树的高度要比B树的高度高

在最坏的情况下磁盘IO的次数由树的高度来决定，所以要减少磁盘IO的次数就必须要压缩树的高度，让瘦高的树尽量变成矮胖的树
